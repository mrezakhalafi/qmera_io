...
if (localStorage.FLAG == "null") {
	document.getElementById("broadcast-modal-toggle").classList.add("d-none");
}
...
let populateChatList = () => {
	...
	MessageUtils.getMessages()
		...
		.forEach((msg) => {
			...
			chat.unread = (msg.sender !== user.id && msg.status < 4 && msg.status > -1) ? 1 : 0;
		...
		.forEach((elem, index) => {
			...
			if (elem.msg.is_deleted == 1) {
				...
				if (elem.msg.file == 'image') {
					...
					messagePreview = elem.msg.body.length <= 25 ? elem.msg.body : elem.msg.body.substr(0, 30) + '...';
				}
			}
...
let addMessageToMessageArea = (msg) => {
	...
	if (msg.reply_to != null && msg.is_deleted == 0) {
		...
		if (replied_msg != undefined && replied_msg.is_deleted != 1) {
			...
			<span class="reply-body" onclick="highlightChat('msg-${msg.reply_to}')" style="cursor:pointer;">
			<div class="d-flex flex-row" style="background-color:${msg.sender == user.id ? "white" : "#dcf8c6"};">
				<div class="body m-1 mr-2"><small><i><i class="fa fa-ban" aria-hidden="true"></i> ${messages.find(el => msg.reply_to == el.id).sender == user.id ? "You deleted this message" : "This message has been deleted."}</i></small></div>
			</div>
			</span>`;
		}
	}
	...
	if (msg.file == 'image') {
		...
		// msgText = typeof msg.body.split('|')[1] === 'undefined' ? msg.body : msg.body.split('|')[1];
	...
	// let timeView = `<div class="time ml-auto small text-right flex-shrink-0 align-self-end text-muted" style="width:75px;">
			...
			if (msg.is_complain == 1) {
				...
				<div class="align-self-${msg.sender === user.id ? "end self" : "start"} p-1 my-1 mx-3 rounded bg-white shadow-sm message-item" id="msg-${msg.id.toString()}">
					<div class="options dropdown">
						<a href="#" class="${msg.is_deleted === 1 || chat.contact.id == "-999" ? "d-none" : ""} dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"> 
							<i class="fas fa-angle-down text-muted px-2"></i>
						</a>
						<div class="dropdown-menu dropdown-menu-right">
							<a class="dropdown-item" onclick="replyMessage('${msg.id}')" href="#">Reply</a>
					...
					<div class="align-self-${msg.sender === user.id ? "end self" : "start"} p-1 my-1 mx-3 rounded bg-white shadow-sm message-item" id="msg-${msg.id.toString()}">
						...
						<div class="d-flex flex-col">
							...
							<div class="body m-1 mr-2">
								...
								${description == null ? "<a href='"+ linkVCR +"'>Join Room</a><br>" : ""}
							</div>
							<div class="time ml-auto small text-right flex-shrink-0 align-self-end text-muted" style="width:75px;">
								${mDate(msg.time).getTime()}
								${(msg.sender === user.id) ? sendStatus : ""}
...
let generateMessageArea = (elem, chatIndex, isNewChat, isGroup) => {
	...
	// if (isOngoingCC == true) {
	// 	if (confirm('You have an ongoing call center session at the moment. Would you like to end the session?')) {
	// 		finishComplain();
	// 	} else {
	// 		return;
	// 	}
...
let sendMessage = () => {
	...
	if (chat.block === '1') {
		...
		return;
	} else if (!chat.hasOwnProperty('contact') && chat.isGroup != true) {
...
DOM.messageInput.addEventListener('keyup', function (e) {
	if (e.key === 'Enter') {
		...
		// let urlRegex = new RegExp("([a-zA-Z0-9]+:\/\/)?([a-zA-Z0-9_]+:[a-zA-Z0-9_]+@)?([a-zA-Z0-9.-]+\\.[A-Za-z]{2,4})(:[0-9]+)?([^ ])+");
		// if (!urlRegex.test(DOM.messageInput.value.trim()) || DOM.messageInput.value.trim() === "") {
		// 	mClassList(DOM.urlPreview).add("d-none");
		// 	msgInput = 0;
		// }
	}
...
let init = () => {
	...
	if (executed === false) {
		...
		xmlHttp.open("get", "/chatcore/logics/fetch_read_messages.php?f_pin=" + localStorage.F_PIN + "&flag= " + localStorage.FLAG);
		xmlHttp.send();
		// generateChatList();
...
// 		}
// 	}
// 	xmlHttp.open("get", "/chatcore/logics/accept_complain.php?f_pin=" + localStorage.F_PIN + "&customer_fpin=" + localStorage.complain + "&ch=" + localStorage.getItem("complain_channel"));
// 	xmlHttp.send();
...
// 		}
// 	}
// 	xmlHttp.open("get", "/chatcore/logics/reject_complain.php?f_pin=" + localStorage.F_PIN + "&customer_fpin=" + localStorage.complain + "&acc=" + 0);
// 	xmlHttp.send();
...
// 		}
// 	}
// 	xmlHttp.open("get", "/chatcore/logics/finish_complain.php?f_pin=" + localStorage.F_PIN + "&customer_fpin=" + localStorage.destination + "&complain_id=" + localStorage.complainID);
// 	xmlHttp.send();
// }
...
// 		}
// 	}
// 	xmlHttp.open("get", "/chatcore/logics/check_complain.php?customer_id=" + localStorage.F_PIN);
// 	xmlHttp.send();
// }
...
// 		}
// 	}
// 	xmlHttp.open("get", "/chatcore/logics/fetch_complaint_status.php?comp_id=" + c_id);
// 	xmlHttp.send();
// }
...
// 	// console.log('accept call: ' + localStorage.getItem('call_complain_ID'));
// 	// console.log('callCCAcc channel: ' + localStorage.getItem('complain_channel'));
...
let replyMessage = (msg_id) => {
	...
	if (message.body == "") {
		...
		document.getElementById("document-name").innerHTML = message.body;
	}
